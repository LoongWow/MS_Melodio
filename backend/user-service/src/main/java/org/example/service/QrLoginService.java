...
public class QrLoginService {
    ...
    public QrKeyResponse generateQrKey() {
        try {
            String url = neteaseApiBaseUrl + "/login/qr/key?timestamp=" + System.currentTimeMillis();
    ...
    public QrImageResponse createQrImage(String key) {
        try {
            String url = neteaseApiBaseUrl + "/login/qr/create?key=" + key + "&qrimg=true&timestamp=" + System.currentTimeMillis();
            String response = restTemplate.getForObject(url, String.class);
    ...
    public LoginResponse completeQrLogin(String cookie) {
        try {
            ...
            // 2. 检查用户是否已存在
            Optional<UserAccount> existingUser = userAccountMapper.findByMusicUserId(userId);
            ...
            return new LoginResponse(userId, "登录成功", nickname);
