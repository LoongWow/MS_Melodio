...
import org.example.repo.RecommendCacheLogMapper;
...
public class MusicApiService {
    ...
    private final RecommendCacheLogMapper recommendCacheLogMapper;
    ...
    public MusicApiService(RestTemplate restTemplate,
                           ...
                           RecommendCacheLogMapper recommendCacheLogMapper,
        ...
        this.recommendCacheLogMapper = recommendCacheLogMapper;
    ...
    public void cacheRecommend(Long userId, List<MusicSongDto> songs) {
        ...
        // 保存到数据库
        UserAccount user = userAccountMapper.findByMusicUserId(userId).orElse(null);
        if (user != null) {
            ...
            recommendCacheLogMapper.save(logEntity);
        }
    }
    ...
    public List<MusicSongDto> getCachedRecommend(Long userId) {
        ...
        if (redisTemplate != null) {
            try {
                ...
                if (value instanceof List<?> list) {
                    return list.stream().map(item -> objectMapper.convertValue(item, MusicSongDto.class)).collect(java.util.stream.Collectors.toList());
                }
            } catch (Exception e) {
        ...
        UserAccount user = userAccountMapper.findByMusicUserId(userId).orElse(null);
        if (user != null) {
            return recommendCacheLogMapper.findTopByUser_IdOrderByCreatedAtDesc(user.getId())
                .map(cacheLog -> {
                    try {
    ...
    public void saveConversation(Long userId, String role, String content) {
        ...
        agentConversationMapper.save(conversation);
    ...
    public List<Map<String, String>> getCachedConversationHistory(Long userId, int limit) {
        ...
        UserAccount user = userAccountMapper.findByMusicUserId(userId).orElse(null);
        if (user != null) {
            List<AgentConversation> conversations = agentConversationMapper
