...
import org.example.repo.MusicKnowledgeMapper;
...
public class RagService {
    ...
    private final MusicKnowledgeMapper knowledgeMapper;
    ...
    public RagService(EmbeddingModel embeddingModel,
                      ...
                      MusicKnowledgeMapper knowledgeMapper) {
    ...
    public void initializeKnowledgeBase() {
        ...
        try {
            List<MusicKnowledge> allKnowledge = knowledgeMapper.findAll();
            if (allKnowledge.isEmpty()) {
                log.info("[RAG] 知识库为空，将创建默认知识");
                createDefaultKnowledge();
                allKnowledge = knowledgeMapper.findAll();
            }
    ...
    public List<KnowledgeDocument> retrieveRelevantKnowledge(String query, int maxResults) {
        try {
            ...
            dev.langchain4j.store.embedding.EmbeddingSearchResult<TextSegment> searchResult = embeddingStore.search(request);
            ...
            log.info("[RAG] 检索完成: query={}, foundCount={}, scores={}",
                query, results.size(),
                results.stream().map(doc -> String.format("%.2f", doc.getRelevanceScore())).collect(java.util.stream.Collectors.toList()));
            return results;
