...
import org.example.repo.AgentConversationMapper;
...
public class ChatOrchestratorService {
    ...
    public AgentReply chat(String message, Long userId) {
        ...
        log.info("[AgentIntent] userId={}, intent={}, message={}", userId, intent.getType(), message);
        if (musicApiService.getPendingSwitchAccount(userId).isPresent()) {
            ...
            log.info("[AgentIntent] userId={}, pendingSwitchAccount=true, forceIntent={}", userId, intent.getType());
        }
        ...
        if (intent.getType() == IntentType.KNOWLEDGE ||
            intent.getType() == IntentType.CHAT ||
            intent.getType() == IntentType.UNKNOWN) {
            ...
            if (!relevantKnowledge.isEmpty()) {
                log.info("[RAG] 意图={}, 检索到 {} 条相关知识", intent.getType(), relevantKnowledge.size());
            }
        } else {
            log.debug("[RAG] 意图={}, 跳过 RAG 检索", intent.getType());
        }
        ...
        // 第6步：处理不同意图
        AgentReply reply = switch (intent.getType()) {
        ...
        // 第7步：保存 AI 回复
        musicApiService.saveConversation(userId, "assistant", reply.getReply());
    ...
    private String loadConversationHistory(Long userId) {
        ...
        List<AgentConversation> history = agentConversationMapper
    ...
    private AgentReply handleSearch(IntentResult intent, String message, Long userId, String systemPrompt) {
        ...
        // 第6步：限制返回数量
        List<MusicSongDto> limitedSongs = filteredSongs.stream().limit(limit).collect(java.util.stream.Collectors.toList());
    ...
    private AgentReply handlePlayByIndex(IntentResult intent, Long userId, String systemPrompt) {
        Long songId = musicTools.getSongFromCache(intent.getIndex(), userId);
        if (songId == null) {
            return new AgentReply("缓存里没有你要的那一首，请先让我搜索一组歌曲。", List.of(), null, null, null, null);
        }
        String reply = chatLanguageModel.generate(UserMessage.from(
            systemPrompt + "\n用户：播放第" + intent.getIndex() + "首。" +
    ...
    private AgentReply handlePlayByKeyword(String message, Long userId, String systemPrompt) {
        ...
        // 第5步：限制返回数量
        List<MusicSongDto> limitedSongs = songs.stream().limit(limit).collect(java.util.stream.Collectors.toList());
    ...
    public AgentReply generateGreeting(Long userId) {
        ...
        // 第5步：搜索推荐歌曲，限制为5首
        List<MusicSongDto> songs = musicTools.searchSongs(keywords, userId);
        List<MusicSongDto> limitedSongs = songs.stream().limit(5).collect(java.util.stream.Collectors.toList());
