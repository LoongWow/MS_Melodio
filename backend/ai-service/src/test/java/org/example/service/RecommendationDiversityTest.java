...
class RecommendationDiversityTest {
    ...
    void recommend_withRecentHistory_shouldExcludePlayedSongs() {
        ...
        assertThat(firstReply.getSongs())
            .as("第一次推荐应该有结果")
            .isNotEmpty();

        // 模拟用户听了前 3 首歌
        List<MusicSongDto> playedSongs = firstReply.getSongs().stream().limit(3).collect(java.util.stream.Collectors.toList());
        ...
        List<Long> secondReplySongIds = secondReply.getSongs().stream()
            .map(MusicSongDto::getId)
            .collect(java.util.stream.Collectors.toList());
    ...
    void recommend_withManyPlayedSongs_shouldReturnAvailableSongs() {
        ...
        // 模拟用户听了几乎所有歌（只留 2 首）
        List<MusicSongDto> allSongs = firstReply.getSongs();
        ...
        assertThat(secondReply.getSongs())
    ...
    void recommend_multipleRequests_shouldProvideDiversity() {
        ...
        assertThat(reply1.getSongs()).isNotEmpty();

        // 模拟用户听了第一批歌曲
        for (MusicSongDto song : reply1.getSongs()) {
            PlayHistory history = new PlayHistory();
            history.setUserId(TEST_USER_ID);
            history.setSongId(song.getId());
            history.setSongName(song.getName());
        ...
        List<Long> songIds1 = reply1.getSongs().stream()
            .map(MusicSongDto::getId)
            .collect(java.util.stream.Collectors.toList());

        List<Long> songIds2 = reply2.getSongs().stream()
            .map(MusicSongDto::getId)
            .collect(java.util.stream.Collectors.toList());
    ...
    void recommend_withIncompleteHistory_shouldNotFilter() {
        ...
        MusicSongDto skippedSong = firstReply.getSongs().get(0);
